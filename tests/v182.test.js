...
test('reset password tidak meminta user ID dan langsung route setelah data rekening lengkap',()=>{
  ...
  assert.match(block,/Jenis rekening:/);
  assert.match(block,/Nama rekening:/);
  assert.match(block,/No rekening:/);
  assert.match(block,/Reply ticket ini dengan User ID\/Username \+ Password/);
});
