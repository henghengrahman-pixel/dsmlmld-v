...
test('deposit flow waits for ID+proof, sends holding, then bridges',()=>{
  ...
  assert.match(engine,/cek deposit apakah sudah diproses atau belum/);
});

test('telegram deposit ticket has done and not found actions',()=>{
  assert.match(bridge,/DP SUDAH/);
  assert.match(bridge,/DP BELUM MASUK/);
});
