...
test('WD selalu diarahkan ke human request',()=>{
  const s=fs.readFileSync(new URL('../src/engine.js',import.meta.url),'utf8');
  assert.match(s,/Every WD problem goes to staff/);
  assert.match(s,/Mohon cek kendala WD member/);
});

test('Human Bridge deposit punya tombol masuk dan belum masuk',()=>{
  const s=fs.readFileSync(new URL('../src/human-bridge.js',import.meta.url),'utf8');
  assert.match(s,/DP MASUK/);
  assert.match(s,/DP BELUM MASUK/);
  assert.match(s,/DP_PROCESSED/);
  assert.match(s,/DP_NOT_FOUND/);
});
