...
test('WD selalu diarahkan ke human request',()=>{
  ...
  assert.match(s,/Mohon cek kendala WD member/);
});

test('Deposit tetap di Tanya Staff panel dan tidak masuk Telegram',()=>{
  ...
  assert.match(e,/telegram:false/);
  assert.match(b,/panel_only/);
  assert.doesNotMatch(b,/DP MASUK/);
  assert.doesNotMatch(b,/DP BELUM MASUK/);
});

test('manual human reply jadi kandidat belajar, bukan auto approve',()=>{
