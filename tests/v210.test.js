...
test('v2.1.1 default inbox follows My chats and excludes closed chats',()=>{
  const c=new LiveChatClient({base:'x',accountId:'a',pat:'p'});
  const items=[
    {id:'1',active:true,routing_status:'active',is_followed:true},
    {id:'2',active:true,routing_status:'active',is_followed:false},
    {id:'3',active:false,routing_status:'closed',is_followed:true}
  ];
  const out=c.filterInbox(items);
  assert.equal(out.some(x=>x.id==='1'),true);
  assert.equal(out.some(x=>x.id==='2'),false);
  assert.equal(out.some(x=>x.id==='3'),false);
});
