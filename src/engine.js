...
async function buildSources(chatId,intent,normalized){
  ...
  return {context,rules,knowledge:[promoKnowledge,manualKnowledge,responses,learning].filter(Boolean).join('\n'),hasKnowledge:Boolean(promoKnowledge||manualKnowledge||responses||learning),attachments,conversationDigest:digest,csStyleExamples,totalMessages:total,caseBrain,digestCovered:digested,digestTarget:target};
}
...
export async function processCustomerMessage({chatId,eventId,text,createdAt,livechat,attachments=[]}) {
  ...
  return db.withChatLock(chatId, async()=>{
    ...
    try {
      ...
      let decision=await ai.classifyAndReply({normalized,intent,context:src.context,rules:src.rules,knowledge:src.knowledge,attachments:aiAttachments,style,conversationDigest:src.conversationDigest,csStyleExamples:src.csStyleExamples,caseBrain:src.caseBrain});
      ...
      if(mergedAttachments.length){
        ...
        if(!currentWf?.workflow_type && /(?:deposit|depo|transfer|bukti\s*(?:tf|transfer))/i.test(hint)){
          const recent=await db.getContext(chatId,40);
          ...
          if(data.userId && data.proofUrl){
            return makeHumanRequest({chatId,eventId,intent:'DEPOSIT_PROBLEM',text,livechat,telegram:true,holding:WAITING_CHECK_REPLY,question:`ID : ${data.userId}
