...
async function maybeHandleOperationalFlow({chatId,eventId,text,intent,livechat}){
  ...
  if(wf?.workflow_state==='WAITING_HUMAN' && ['WD_CHECK','DEPOSIT_VERIFY','PROOF_REVIEW','RESET_PASSWORD','BONUS_CLAIM','FREEBET_CLAIM','PAYOUT_CHECK','ACCOUNT_CHANGE','ACCOUNT_LIMIT','GAME_CHECK','ACCESS_CHECK','REGISTER_CHECK','LOSS_REVIEW'].includes(String(wf?.workflow_type||''))){
    const label={WD_CHECK:'WD',DEPOSIT_VERIFY:'deposit',PROOF_REVIEW:'bukti/foto',RESET_PASSWORD:'reset password',BONUS_CLAIM:'bonus',FREEBET_CLAIM:'bonus FreeBet',PAYOUT_CHECK:'kemenangan/payout',ACCOUNT_CHANGE:'ganti rekening',ACCOUNT_LIMIT:'rekening limit',GAME_CHECK:'permainan',ACCESS_CHECK:'akses',REGISTER_CHECK:'pendaftaran',LOSS_REVIEW:'keluhan'}[wf.workflow_type]||'permintaan';
...
reset password ko`,requireTelegramDelivery:true});
  ...
  if(wf?.workflow_type==='WD_CHECK' && wf?.workflow_state==='WAITING_ID'){
    ...
    const result=await makeHumanRequest({
      ...
      holding:WD_WAITING_STAFF_REPLY,
