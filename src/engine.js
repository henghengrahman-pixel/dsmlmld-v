...
async function maybeHandleOperationalFlow({chatId,eventId,text,intent,livechat}){
  ...
  if(wf?.workflow_state==='WAITING_HUMAN' && ['WD_CHECK','DEPOSIT_VERIFY','RESET_PASSWORD','BONUS_CLAIM','FREEBET_CLAIM','PAYOUT_CHECK','ACCOUNT_CHANGE','ACCOUNT_LIMIT','GAME_CHECK','ACCESS_CHECK','REGISTER_CHECK','LOSS_REVIEW'].includes(String(wf?.workflow_type||''))){
    const label={WD_CHECK:'WD',DEPOSIT_VERIFY:'deposit',RESET_PASSWORD:'reset password',BONUS_CLAIM:'bonus',FREEBET_CLAIM:'bonus FreeBet',PAYOUT_CHECK:'kemenangan/payout',ACCOUNT_CHANGE:'ganti rekening',ACCOUNT_LIMIT:'rekening limit',GAME_CHECK:'permainan',ACCESS_CHECK:'akses',REGISTER_CHECK:'pendaftaran',LOSS_REVIEW:'keluhan'}[wf.workflow_type]||'permintaan';
...
reset password ko`,requireTelegramDelivery:true});
  ...
  if(wf?.workflow_type==='WD_CHECK' && wf?.workflow_state==='WAITING_ID'){
    const uid=extractUserIdFromText(text) || extractRequestedUserIdFromText(text) || extractUserId(ctx) || extractStandaloneRequestedUserId(ctx);
...
export async function processCustomerMessage({chatId,eventId,threadId=null,text,createdAt,livechat,attachments=[],skipInsert=false,resumeReason=''}) {
  ...
  return db.withChatLock(chatId, async()=>{
    ...
    if(isAcknowledgementText(text)){
      const reply=acknowledgementReply();
      await sendAndStore(livechat,chatId,reply,'ACKNOWLEDGEMENT');
      await db.logAI({chatId,sourceEventId:eventId,intent:'ACKNOWLEDGEMENT',action:'AUTO_REPLY',confidence:1,reply,reason:'short_ack_does_not_retrigger_operational_workflow'}).catch(()=>{});
