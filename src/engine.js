...
function parseResetCredentialReply(text=''){
  ...
  const link=(s.match(/(?:link(?:\s*login)?)\s*[:=]\s*(https?:\/\/\S+)/i)||s.match(/https?:\/\/\S+/i)||[])[1]?.trim?.()||'';
  if(user && password) return {userId:user,password,link};
  // Normal CS shorthand supported by request: exactly reply to ticket with
  // USER ID on line 1, PASSWORD on line 2, LINK on line 3.
  const lines=s.split(/\r?\n/).map(x=>x.trim()).filter(Boolean);
  if(lines.length>=2 && !/^(?:done|belum|pending)$/i.test(lines[0])){
    const possibleLink=lines.find((x,i)=>i>=2 && /^https?:\/\//i.test(x))||'';
    if(lines[0].length<=120 && lines[1].length<=160) return {userId:lines[0],password:lines[1],link:possibleLink};
  }
  return null;
}
...
export async function answerHumanRequest({request,humanAnswer,saveAsKnowledge=false,livechat}){
  return db.withChatLock(request.chat_id, async()=>{
    ...
    if(intent==='FORGOT_PASSWORD'){
      ...
      if(!credentials){
        const e=new Error('RESET_REPLY_INCOMPLETE: reply ticket dengan USER ID, PASSWORD, dan LINK (link bila tersedia).'); e.code='RESET_REPLY_INCOMPLETE'; throw e;
      }
