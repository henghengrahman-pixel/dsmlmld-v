...
async function resolveBonusLabel(text='',siteId='default'){
  const n=normalizeText(text);
  // Menu Penting is the newest source and supports promo names added by admin without code changes.
  const important=(await db.getRelevantImportantInfo(n,30,siteId)).filter(x=>String(x.item_type||'').toUpperCase()==='PROMO');
  if(important.length) return String(important[0].title||important[0].item_key||'').toUpperCase().slice(0,100);
  const promos=await db.listPromoRules({activeOnly:true,limit:200,siteId});
...
function workflowRelevant(wf,intent='GENERAL'){
  if(!wf?.workflow_type) return false;
  const i=String(intent||'GENERAL').toUpperCase();
  // Ambiguous/short follow-ups (ID, number, proof caption, "sudah") continue the active workflow.
  if(['GENERAL','GREETING','UNKNOWN'].includes(i)) return true;
  const t=String(wf.workflow_type||'').toUpperCase();
  if(t.startsWith('DEPOSIT')) return i.includes('DEPOSIT');
  if(t.startsWith('RESET')) return i.includes('PASSWORD')||i.includes('RESET')||i==='FORGOT_PASSWORD';
  if(t.startsWith('BONUS')) return i.includes('BONUS');
  if(t.startsWith('WD')||t.includes('WITHDRAW')) return i.includes('WITHDRAW')||i.startsWith('WD_');
  return false;
}
...
async function maybeHandleOperationalFlow({chatId,eventId,text,intent,livechat}){
  const storedWf=await db.getConversationWorkflow(chatId);
  const ctx=await getOperationalHistory(chatId);
  const effective=String(intent||'GENERAL').toUpperCase();
  const wf=workflowRelevant(storedWf,effective)?storedWf:null;
  ...
  if(['LINK_ACCESS','RTP_INFO','PREDIKSI_TOGEL'].includes(effective)){
    ...
    const rows=(await db.getRelevantImportantInfo(text,20,await db.getConversationSite(chatId))).filter(x=>String(x.item_type||'').toUpperCase()===wanted);
    if(rows.length){
      const best=rows[0];
  ...
  if(['ABUSIVE','COMPLAINT','LOSS_COMPLAINT'].includes(effective)){
    ...
    if(effective==='ABUSIVE' && count>=3){
      const reply=await responseText('#KOMPLAIN_MAKI_ULANG','Mohon maaf bosku 🙏 Oke bosku, kalau ada kendala yang mau dibantu cek kabari kami ya.',chatId);
    ...
    if(effective==='LOSS_COMPLAINT'){
      const base=await responseText('#KOMPLAIN_KALAH','Mohon maaf ya bosku 🙏 Kalau ada kendala di permainan atau transaksi, bilang bagian mana yang bermasalah biar kami bantu cek.',chatId);
      const rtp=await db.getCannedByShortcut('#RTP',await db.getConversationSite(chatId));
    ...
    const reply=await responseText('#KOMPLAIN_MAKI','Mohon maaf ya bosku 🙏 Ada kendala apa yang bisa kami bantu cek?',chatId);
    await sendAndStore(livechat,chatId,reply,effective);
...
reset password ko`});
  ...
  if(effective==='BONUS_INFO'){
    const matched=(await db.getRelevantImportantInfo(text,12,await db.getConversationSite(chatId))).filter(x=>String(x.item_type||'').toUpperCase()==='PROMO');
    const n=normalizeText(text);
    ...
    if(matched.length && !/^(bonus|promo|event)( apa| apa saja| apa aja| apa yang ada| saja| aja)?$/i.test(n)){
      ...
      const all=await db.listImportantInfo({activeOnly:true,type:'PROMO',limit:100,siteId:await db.getConversationSite(chatId)});
      if(all.length){
        ...
        reply=await responseText('#BONUS_MINGGUAN','Untuk informasi promo terbaru, boleh tunggu sebentar ya bosku. Kami cek dulu promo yang sedang aktif.',chatId);
  ...
  if(effective==='BONUS_REQUEST' && wf?.workflow_type!=='BONUS_CLAIM'){
    const bonusType=await resolveBonusLabel(text,await db.getConversationSite(chatId));
    if(!bonusType){
      return askAndTrack(livechat,chatId,'BONUS_REQUEST','BONUS_CLAIM','ASK_TYPE',{},await responseText('#BONUS_TANYA','Bonus apa yang mau diklaim ya bosku? 😊',chatId));
    }
  }
  if(wf?.workflow_type==='BONUS_CLAIM' && wf?.workflow_state==='ASK_TYPE'){
    const bonusType=await resolveBonusLabel(text,await db.getConversationSite(chatId)) || String(text||'').trim().slice(0,120);
  ...
  if(effective.includes('BONUS')){
    const bonusType=await resolveBonusLabel(text,await db.getConversationSite(chatId)) || effective.replace(/^BONUS_?/,'').replaceAll('_',' ');
    const uid=extractUserIdFromText(text) || extractClaimUserIdFromText(text) || extractUserId(ctx);
...
async function responseText(shortcut,fallback,chatId=''){
  const siteId=chatId?await db.getConversationSite(chatId):'default';
  const r=await db.getCannedByShortcut(normalizeShortcut(shortcut),siteId);
...
async function maybeHandleWorkflow({chatId,eventId,text,intent,livechat}){
  const storedWf=await db.getConversationWorkflow(chatId);
  const wf=workflowRelevant(storedWf,intent)?storedWf:null;
  ...
  if(wf?.workflow_type==='WD_STATUS' && wf?.workflow_state==='PENDING'){
    const reply=await responseText('#WD_ANTRIAN','Withdraw bosku masih dalam proses ya 😊🙏 Mohon ditunggu beberapa saat, nanti tetap kami proses sampai selesai ya bosku.',chatId);
...
async function buildSources(chatId,intent,normalized){
  ...
  const importantRows=await db.getRelevantImportantInfo(query,24,siteId); // compatibility: getRelevantImportantInfo(query,24)
  ...
  const historyRows=await db.getAutoHistoryLearning(query,intent,12,siteId);
  const styleRows=await db.getHumanStyleExamples(30,siteId); // compatibility: getHumanStyleExamples(30)
  ...
  const profile=await db.getSiteProfile(chatId);
  const siteProfileKnowledge=profile&&Object.keys(profile).length>1?`[WEBSITE AKTIF] ${profile.name||profile.siteId} | login:${profile.loginUrl||'-'} | register:${profile.registerUrl||'-'} | catatan:${profile.notes||'-'}`:'';
...
export async function processCustomerMessage({chatId,eventId,text,createdAt,livechat,attachments=[],reprocessExisting=false}) {
  ...
  if(!['GREETING','GENERAL'].includes(String(intent||'').toUpperCase())) await db.ensureActiveCase(chatId,intent,{sourceEventId:eventId,summary:String(text||'').slice(0,800)}).catch(()=>{});
  ...
  return db.withChatLock(chatId, async()=>{
    ...
    const openHuman=await db.getOpenHumanRequest(chatId,intent);
...
export async function answerHumanRequest({request,humanAnswer,saveAsKnowledge=false,livechat}){
  return db.withChatLock(request.chat_id, async()=>{
    ...
    if(intent==='WITHDRAW_PROBLEM' && /\bdana\b.*\blimi(?:t|d)\b|\blimi(?:t|d)\b.*\bdana\b/i.test(String(humanAnswer||''))){
      const reply=await responseText('#DANA_LIMIT',WD_DANA_LIMIT_REPLY,request.chat_id);
...
export async function applyHumanAction({request,action,livechat}){
  ...
  return db.withChatLock(request.chat_id, async()=>{
    ...
    if(code==='RESET_NOT_REGISTERED'){
      const profile=await db.getSiteProfile(request.chat_id);
      const reg=String(profile.registerUrl||'').trim();
      const fallback=['Mohon maaf ya, bosku. Setelah kami cek, data yang diberikan belum terdaftar di situs kami 🙏😊','Jika bosku berminat, kami bisa bantu proses pendaftaran akun baru.',reg?`🔗 LINK PENDAFTARAN:\n${reg}`:'Silakan minta link pendaftaran terbaru kepada CS.','Silakan dicoba ya, bosku. Kami siap membantu jika ada kendala saat pendaftaran ☺️🙏'].join('\n\n');
      const reply=await responseText('#RESET_TIDAK_TERDAFTAR',fallback,request.chat_id);
    ...
    if(code==='RESET_DEPOSIT_FIRST'){
      ...
      const deposit=await db.findDepositResponseForBank(data.type||'',await db.getConversationSite(request.chat_id));
      const intro=await responseText('#RESET_DEPOSIT_DULU','Untuk verifikasi kepemilikan akun, silakan melakukan deposit melalui rekening tujuan di bawah ini ya bosku 🙏 Setelah deposit selesai, kirim bukti transfernya kembali kepada kami agar proses reset password dapat dilanjutkan.',request.chat_id);
    ...
    if(code==='RESET_DEPOSIT_NOT_IN'){
      const reply=await responseText('#RESET_DP_BELUM_MASUK','Deposit verifikasinya belum terlihat masuk ya bosku 🙏 Mohon cek kembali transfernya. Jika sudah, kirim kembali bukti verifikasi yang terbaru kepada kami.',request.chat_id);
      await sendAndStore(livechat,request.chat_id,reply,request.intent||'FORGOT_PASSWORD');
    ...
    if(code==='DP_DETAIL_PROOF'){
      const reply=await responseText('#DP_DETAIL_PROOF','Silakan dibantu dengan Detail Bukti transfernya ya bosku 😊 Pastikan terlihat jelas waktu transaksi, tanggal transaksi, nominal, dan RRN / nomor referensi. Setelah itu kirim kembali kepada kami ya bosku.',request.chat_id);
    ...
    if(code==='DP_UNCLEAR_PROOF'){
      const reply=await responseText('#DP_UNCLEAR_PROOF','Bukti yang dikirim belum terlihat jelas ya bosku 🙏 Mohon difoto ulang atau screenshot kembali dengan jelas supaya detail transaksinya dapat kami bantu cek.',request.chat_id);
      await sendAndStore(livechat,request.chat_id,reply,request.intent||'DEPOSIT_PROBLEM');
      const ctx=await db.getContext(request.chat_id,80); const userId=extractUserId(ctx); const previousProofUrl=latestProof(ctx)?.url||'';
    ...
    const [shortcut,fallback]=map[code]; const reply=await responseText(shortcut,fallback,request.chat_id);
