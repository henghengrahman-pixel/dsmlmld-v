...
async function buildSources(chatId,intent,normalized){
  ...
  return {context,rules,knowledge:[manualKnowledge,responses,learning].filter(Boolean).join('\n'),hasKnowledge:Boolean(manualKnowledge||responses||learning),attachments,conversationDigest:digest,csStyleExamples,totalMessages:total,caseBrain,digestCovered:digested,digestTarget:target};
}
