...
async function buildSources(chatId,intent,normalized){
  ...
  return {context,rules,knowledge:[promoKnowledge,manualKnowledge,responses,learning].filter(Boolean).join('\n'),hasKnowledge:Boolean(promoKnowledge||manualKnowledge||responses||learning),attachments,conversationDigest:digest,csStyleExamples,historyLearning,totalMessages:total,caseBrain,digestCovered:digested,digestTarget:target};
}
