...
async function maybeHandleOperationalFlow({chatId,eventId,text,intent,livechat}){
  ...
  if(effective==='FORGOT_PASSWORD' || wf?.workflow_type==='RESET_PASSWORD'){
    ...
    await db.clearConversationWorkflow(chatId);
    return makeHumanRequest({chatId,eventId,intent:'FORGOT_PASSWORD',text,livechat,telegram:true,holding:'Siap bosku, datanya sudah lengkap 🙏 Kami bantu proses reset passwordnya ya.',question:`NO REK ${data.no}\na/n${data.name}\nJENIS REK : ${data.type}\n\nreset password ko`});
  }
  ...
  if(effective==='WITHDRAW_PROBLEM'){
    ...
    return makeHumanRequest({chatId,eventId,intent:'WITHDRAW_PROBLEM',text,livechat,telegram:true,holding:'Siap bosku, kami bantu teruskan pengecekan WD-nya dulu ya 🙏',question:`${uid?`ID : ${uid}\n\n`:''}kendala wd\n${all.slice(-700)}`});
  }
  ...
  if(wf?.workflow_type==='BONUS_CLAIM' && wf?.workflow_state==='ASK_TYPE'){
    ...
    await db.clearConversationWorkflow(chatId);
    return makeHumanRequest({chatId,eventId,intent:'BONUS_REQUEST',text,livechat,telegram:true,holding:`Siap bosku, kami bantu cek bonus ${bonusType} dulu ya 🙏`,question:`ID : ${uid}\n\nclaim bonus ${String(bonusType).toLowerCase()}`});
  }
  ...
  if(effective.includes('BONUS')){
    ...
    await db.clearConversationWorkflow(chatId);
    return makeHumanRequest({chatId,eventId,intent:effective,text,livechat,telegram:true,holding:`Siap bosku, kami bantu cek bonus ${bonusType||''} dulu ya 🙏`,question:`ID : ${uid}\n\nclaim bonus ${String(bonusType||'').toLowerCase()}`});
  }
  ...
  if(effective==='ACCOUNT_CHANGE_REQUEST'){
    const uid=extractUserId(ctx);
    return makeHumanRequest({chatId,eventId,intent:'ACCOUNT_CHANGE_REQUEST',text,livechat,telegram:true,holding:'Siap bosku, permintaan ganti rekeningnya kami teruskan ya 🙏',question:`${uid?`ID : ${uid}\n\n`:''}minta ganti rekening`});
  }
  return null;
}
...
async function maybeHandleWorkflow({chatId,eventId,text,intent,livechat}){
  ...
  const req=await db.createHumanRequest({
    ...
    question:`${String(text||'').slice(0,1200)}
