...
export async function answerHumanRequest({request,humanAnswer,saveAsKnowledge=false,livechat}){
  ...
  const style=await getReplyStyle();
  const composed=await ai.composeFromHuman({memberMessage:request.member_message,intent:request.intent||'GENERAL',humanAnswer,context:src.context,rules:src.rules,knowledge:src.knowledge,style});
