...
${all.slice(-900)}`,requireTelegramDelivery:true});
  ...
  // Game macet/error: minta ID jika belum ada, lalu WAJIB Telegram.
  if(effective==='GAME_PROBLEM' || wf?.workflow_type==='GAME_CHECK'){
    ...
    const uid=extractUserIdFromText(text) || extractUserId(ctx) || (workflowAskedForId?extractStandaloneRequestedUserId(ctx):'');
    if(!uid) return askAndTrack(livechat,chatId,'GAME_PROBLEM','GAME_CHECK','WAITING_ID',{},'Boleh kirim ID akunnya ya bosku 🙏 Biar kendala gamenya kami teruskan ke staff untuk dicek.');
    const all=customerTexts(ctx).slice(-8).join(' | ');
...
ID : ${uid}
${all.slice(-900)}`,requireTelegramDelivery:true});
    if(result?.telegram) await db.setConversationWorkflow(chatId,{type:'GAME_CHECK',state:'WAITING_HUMAN',data:{userId:uid,humanRequestId:result.humanRequestId}});
    return result;
  }
  return null;
}
