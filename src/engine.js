...
async function maybeHandleOperationalFlow({chatId,eventId,text,intent,livechat}){
  ...
  // Deposit complaint: collect ID + proof, then ask staff in PANEL ONLY.
  // Telegram Bridge is intentionally limited to Reset Password, Bonus, and WD.
  if(effective==='DEPOSIT_PROBLEM' || wf?.workflow_type==='DEPOSIT_VERIFY'){
    ...
    return makeHumanRequest({chatId,eventId,intent:'DEPOSIT_PROBLEM',text,livechat,telegram:false,holding:'Siap bosku, user ID dan bukti transfernya sudah kami terima 🙏 Kami bantu cek depositnya dulu ya.',question:`Mohon cek deposit member.\nUser ID: ${data.userId}\nBukti transfer: ${data.proofUrl}\nBalas di panel: jika dana masuk tulis bahwa deposit sudah diproses; jika belum terlihat, beri instruksi sesuai hasil pengecekan.`});
  }

  // Reset password: NEVER ask user ID. Collect only registered account type/name/number, then route to RESET PASSWORD Telegram group.
  if(effective==='FORGOT_PASSWORD' || wf?.workflow_type==='RESET_PASSWORD'){
    const acc=extractAccountData(ctx); const prev=wf?.workflow_data||{};
    const data={type:acc.type||prev.type||'',name:acc.name||prev.name||'',no:acc.no||prev.no||''};
    ...
    await db.clearConversationWorkflow(chatId);
    return makeHumanRequest({chatId,eventId,intent:'FORGOT_PASSWORD',text,livechat,telegram:true,holding:'Siap bosku, datanya sudah lengkap 🙏 Kami bantu proses reset passwordnya ya.',question:`Mohon proses reset password member.\nJenis rekening: ${data.type}\nNama rekening: ${data.name}\nNo rekening: ${data.no}\nReply ticket ini dengan User ID/Username + Password (+ link login bila ada). Balasan akan diteruskan hanya ke member pada ticket ini.`});
  }

  // Every WD problem goes to Telegram staff.
  if(effective==='WITHDRAW_PROBLEM'){
    const uid=extractUserId(ctx); const all=customerTexts(ctx).slice(-5).join(' | ');
    return makeHumanRequest({chatId,eventId,intent:'WITHDRAW_PROBLEM',text,livechat,telegram:true,holding:'Siap bosku, kami bantu teruskan pengecekan WD-nya dulu ya 🙏',question:`Mohon cek kendala WD member.${uid?`\nUser ID: ${uid}`:''}\nKonteks terbaru: ${all.slice(0,1200)}`});
  }

  // Generic bonus claim MUST ask which bonus first.
  ...
  if(wf?.workflow_type==='BONUS_CLAIM' && wf?.workflow_state==='ASK_TYPE'){
    ...
    await db.clearConversationWorkflow(chatId);
    return makeHumanRequest({chatId,eventId,intent:'BONUS_REQUEST',text,livechat,telegram:true,holding:`Siap bosku, kami bantu cek bonus ${bonusType} dulu ya 🙏`,question:`Mohon cek kelayakan/proses bonus member.\nBonus yang diminta: ${bonusType}\nKonteks terbaru: ${customerTexts(ctx).slice(-5).join(' | ').slice(0,1200)}`});
  }

  // Specific bonus, such as BONUS_DAILY, goes directly to Bonus Telegram group.
  if(effective.includes('BONUS')){
    const bonusType=pickBonusLabel(text) || effective.replace(/^BONUS_?/,'').replaceAll('_',' ');
    const uid=extractUserId(ctx); const all=customerTexts(ctx).slice(-5).join(' | ');
    return makeHumanRequest({chatId,eventId,intent:effective,text,livechat,telegram:true,holding:`Siap bosku, kami bantu cek bonus ${bonusType||''} dulu ya 🙏`,question:`Mohon cek kelayakan/proses bonus member.${bonusType?`\nBonus: ${bonusType}`:''}${uid?`\nUser ID: ${uid}`:''}\nKonteks terbaru: ${all.slice(0,1200)}`});
  }
  return null;
}
    ...
    try {
      ...
      if(['ASK_HUMAN','HANDOFF'].includes(decision.action)){
        ...
        if(humanAsk){
          ...
          await db.logAI({chatId,sourceEventId:eventId,intent,...decision,reply:'',reason:`${decision.reason||''} | human_request:${req.id} | silent_wait_staff`});
