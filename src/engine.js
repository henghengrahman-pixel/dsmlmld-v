...
async function maybeHandleOperationalFlow({chatId,eventId,text,intent,livechat}){
  ...
  if(wf?.workflow_state==='WAITING_HUMAN' && ['WD_CHECK','DEPOSIT_VERIFY','RESET_PASSWORD','BONUS_CLAIM'].includes(String(wf?.workflow_type||''))){
    const label={WD_CHECK:'WD',DEPOSIT_VERIFY:'deposit',RESET_PASSWORD:'reset password',BONUS_CLAIM:'bonus'}[wf.workflow_type]||'permintaan';
