...
export async function processCustomerMessage({chatId,eventId,text,createdAt,livechat}) {
  ...
  const cannedRows=await db.getRelevantCanned(`${normalized} ${contextRows.filter(m=>m.sender_type==='customer').slice(-3).map(m=>m.text).join(' ')}`,8);
  ...
  const cannedKnowledge=cannedRows.map(r=>`[LIVECHAT RESPONSE ${r.shortcut||r.source_id}] ${r.content}`).join('\n');
  const knowledge=[manualKnowledge,cannedKnowledge].filter(Boolean).join('\n');
  try {
    ...
    if (sentEventId) await db.insertMessage({chatId,eventId:String(sentEventId),senderType:'ai',authorId:'',text:decision.reply,normalizedText:normalizeText(decision.reply),intent,createdAt:new Date().toISOString()});
    await db.logAI({chatId,sourceEventId:eventId,intent,...decision});
