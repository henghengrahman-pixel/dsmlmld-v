...
import { migrate, healthDb, pool, getSetting, setSetting, getContext, setHumanTakeover, clearHumanTakeover, logError } from './db.js';
...
app.post('/api/settings/auto-reply',requireAdmin,async(req,res)=>{const enabled=Boolean(req.body?.enabled);await setSetting('auto_reply',enabled);res.json({ok:true,enabled});});

app.get('/api/conversations',requireAdmin,async(req,res)=>{const r=await pool.query(`SELECT chat_id,customer_name,customer_email,status,human_takeover_until,last_event_at,last_member_event_at,last_ai_event_at,updated_at FROM conversations ORDER BY COALESCE(last_event_at,updated_at) DESC LIMIT 200`);res.json({ok:true,items:r.rows});});
...
app.post('/api/conversations/:id/send',requireAdmin,async(req,res)=>{const text=String(req.body?.text||'').trim();if(!text)return res.status(400).json({ok:false,error:'EMPTY_TEXT'});try{const sent=await lc.sendMessage(req.params.id,text);res.json({ok:true,sent});}catch(e){res.status(502).json({ok:false,error:e.message});}});
