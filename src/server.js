...
import { migrate, healthDb, pool, getSetting, setSetting, getContext, setHumanTakeover, clearHumanTakeover, withChatLock, isHumanTakeover, logError, insertMessage, addAiCorrection, backfillHumanLearning, backfillHumanLearningAll, getConversationBrain, listLearningExamples, setLearningStatus, promoteLearningToKnowledge, promoteLearningToResponse, captureHumanReplyLearning, markConversationEnded, learningStats, getConversationState, getFullContext, markAiMessageFeedback, aiFeedbackStats, getIntegrationHealth, listCaseAudit, setIntegrationHealth } from './db.js';
import { LiveChatClient } from './livechat.js';
import { OpenAIClient } from './ai.js';
import { normalizeText, detectIntent } from './normalizer.js';
import { createSession, refreshSession, createPending2fa, verifyPending2fa, generateTotpSecret, verifyTotp, makeOtpAuthUrl, safeEqualText, sessionCookie, clearSessionCookie, parseCookies, verifySession, requireAdmin } from './auth.js';
...
app.get('/health', async(req,res)=>{
  ...
  const integrations=db?await getIntegrationHealth().catch(()=>[]):[];
  res.status(db?200:503).json({ok:db,service:'livechat-ai',db,livechatConfigured:lc.ready(),openaiConfigured:ai.ready(),poller:pollerStatus(),integrations,warnings:validateConfig()});
});
...
app.post('/api/login',async(req,res)=>{
  try{
    ...
    const ok=safeEqualText(String(username||''),String(config.adminUsername||'')) && safeEqualText(String(password||''),String(config.adminPassword||''));
...
app.post('/api/login/2fa/setup/verify',async(req,res)=>{
  try{
    ...
    if(!verifyTotp(state.secret,req.body?.code||'')) return res.status(401).json({ok:false,error:'TWOFA_INVALID'});
...
app.get('/api/conversations/:id',requireAdmin,async(req,res)=>{const full=String(req.query.full||'')==='1';const [items,state,brain]=await Promise.all([full?getFullContext(req.params.id,20000):getContext(req.params.id,100),getConversationState(req.params.id),getConversationBrain(req.params.id)]);res.json({ok:true,items,state,brain:brain?.case_brain||{},feedbackStats:await aiFeedbackStats(),full});});
...
app.post('/api/learning/backfill',requireAdmin,async(req,res)=>{
  try{
    const maxRows=Math.max(1000,Math.min(Number(req.body?.limit||50000),100000));
    const result=await backfillHumanLearningAll({batchSize:2000,maxRows});
    res.json({ok:true,...result});
  }catch(e){
    const message=String(e?.message||e||'');
    if(String(e?.code||'')==='STALE_SESSION_REQUEST' || /STALE_SESSION_REQUEST/i.test(message)){
      await logError('learning_scan','STALE_HISTORY_SKIPPED',message,{route:'/api/learning/backfill'}).catch(()=>{});
      return res.json({ok:true,scanned:0,created:0,skipped:1,staleSkipped:1,failed:0,batches:0,errors:[]});
    }
