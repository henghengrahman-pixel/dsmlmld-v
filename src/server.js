...
app.use((req,res,next)=>{req.siteId='default';next();});
...
app.post('/webhooks/livechat',async(req,res)=>{
  try{
    const p=req.body||{};
    const requestedSite='default';
    const webhookSecret=lc.getWebhookSecret();
    if(webhookSecret){
    ...
    const chatId=nativeChatId; const eventId=String(ev.id||crypto.createHash('sha1').update(`${nativeChatId}|${ev.created_at||''}|${ev.text}`).digest('hex'));
