...
import { migrate, healthDb, pool, getSetting, setSetting, getContext, setHumanTakeover, clearHumanTakeover, withChatLock, isHumanTakeover, logError, insertMessage, addAiCorrection, listLearningExamples, setLearningStatus, promoteLearningToKnowledge, promoteLearningToResponse } from './db.js';
...
import { cannedStats, listCannedResponses, createManualResponse, updateManualResponse, deleteManualResponse, importManualResponses, listHumanRequests, getHumanRequest, cancelHumanRequest, getTelegramSettingsInternal, saveTelegramSettings, setTelegramEnabled, listBridgeTickets, closeBridgeTicketByRequest } from './db.js';
...
app.get('/api/conversations',requireAdmin,async(req,res)=>{const r=await pool.query(`SELECT chat_id,customer_name,customer_email,status,handling_mode,takeover_reason,takeover_at,last_event_at,last_member_event_at,last_ai_event_at,workflow_type,workflow_state,member_typing,member_typing_updated_at,updated_at FROM conversations WHERE visible_in_inbox=true ORDER BY COALESCE(last_event_at,updated_at) DESC LIMIT 200`);res.json({ok:true,items:r.rows});});
...
app.post('/api/conversations/:id/send',requireAdmin,async(req,res)=>{const text=String(req.body?.text||'').trim();if(!text)return res.status(400).json({ok:false,error:'EMPTY_TEXT'});try{const out=await withChatLock(req.params.id,async()=>{if(!(await isHumanTakeover(req.params.id))) { const er=new Error('HUMAN_TAKEOVER_REQUIRED'); er.status=409; throw er; } const sent=await lc.sendMessage(req.params.id,text);const eventId=sent?.event_id||sent?.id||null;if(eventId)await insertMessage({chatId:req.params.id,eventId:String(eventId),senderType:'agent',authorId:'admin',text,normalizedText:normalizeText(text),intent:detectIntent(text),createdAt:new Date().toISOString()});return {ok:true,sent};});res.json(out);}catch(e){res.status(e.status||502).json({ok:false,error:e.message});}});


app.get('/api/learning',requireAdmin,async(req,res)=>{
  const status=String(req.query.status||'ALL').toUpperCase();
  res.json({ok:true,items:await listLearningExamples(status,400)});
});
