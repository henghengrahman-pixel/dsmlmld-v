...
import { cannedClient, startCannedSync, syncCannedNow, cannedSyncStatus, testCannedSites } from './canned-sync.js';
...
const app=express(); const lc=new LiveChatHub(); const ai=new OpenAIClient();
app.disable('x-powered-by');
app.use(express.json({limit:'1mb',verify:(req,res,buf)=>{req.rawBody=Buffer.from(buf)}}));
app.use(express.urlencoded({extended:false}));
app.use((req,res,next)=>{const raw=String(req.headers['x-site-id']||req.query?.site||'default');req.siteId=raw.toLowerCase().replace(/[^a-z0-9_-]+/g,'-')||'default';next();});
...
app.post('/api/test/canned',requireAdmin,async(req,res)=>{try{res.json(await testCannedSites(req.siteId));}catch(e){await logError('canned','TEST_FAILED',e.message);res.status(502).json({ok:false,error:e.message,hint:e.status===403?'PAT belum memiliki izin membaca canned responses/Responses List. Tambahkan read scope untuk canned responses di Text Developer Console.':null});}});
app.post('/api/canned/sync',requireAdmin,async(req,res)=>{try{res.json(await syncCannedNow({manual:true,siteId:req.siteId}));}catch(e){res.status(502).json({ok:false,error:e.message,hint:e.status===403?'Tambahkan read scope canned responses ke PAT, lalu buat PAT baru dan update LIVECHAT_PAT.':null});}});
app.get('/api/canned',requireAdmin,async(req,res)=>{res.json({ok:true,stats:await cannedStats(req.siteId),items:await listCannedResponses(1000,req.siteId)});});
app.post('/api/canned/manual',requireAdmin,async(req,res)=>{try{const {shortcut='',title='',category='GENERAL',content='',tags=[],mode='FLEXIBLE'}=req.body||{};if(!String(content).trim())return res.status(400).json({ok:false,error:'CONTENT_REQUIRED'});const item=await createManualResponse({shortcut:String(shortcut).trim(),title:String(title).trim(),category,content:String(content).trim(),tags:Array.isArray(tags)?tags:String(tags||'').split(',').map(x=>x.trim()).filter(Boolean),mode,siteId:req.siteId});res.json({ok:true,item});}catch(e){res.status(500).json({ok:false,error:e.message});}});
app.put('/api/canned/manual/:id',requireAdmin,async(req,res)=>{try{const item=await updateManualResponse(req.params.id,req.body||{},req.siteId);if(!item)return res.status(404).json({ok:false,error:'RESPONSE_NOT_FOUND'});res.json({ok:true,item});}catch(e){res.status(500).json({ok:false,error:e.message});}});
app.delete('/api/canned/manual/:id',requireAdmin,async(req,res)=>{res.json({ok:await deleteManualResponse(req.params.id,req.siteId)});});
app.post('/api/canned/import',requireAdmin,async(req,res)=>{try{const items=Array.isArray(req.body?.items)?req.body.items:[];if(!items.length)return res.status(400).json({ok:false,error:'ITEMS_REQUIRED'});const created=await importManualResponses(items.slice(0,1000).map(x=>({...x,siteId:req.siteId})));res.json({ok:true,created:created.length});}catch(e){res.status(500).json({ok:false,error:e.message});}});

app.get('/api/promos',requireAdmin,async(req,res)=>{res.json({ok:true,items:await listPromoRules({activeOnly:false,limit:500,siteId:req.siteId})});});
app.post('/api/promos',requireAdmin,async(req,res)=>{try{res.json({ok:true,item:await createPromoRule({...req.body,siteId:req.siteId})});}catch(e){res.status(400).json({ok:false,error:e.message});}});
app.put('/api/promos/:id',requireAdmin,async(req,res)=>{try{const item=await updatePromoRule(req.params.id,req.body||{},req.siteId);if(!item)return res.status(404).json({ok:false,error:'PROMO_NOT_FOUND'});res.json({ok:true,item});}catch(e){res.status(400).json({ok:false,error:e.message});}});
app.delete('/api/promos/:id',requireAdmin,async(req,res)=>{res.json({ok:await deletePromoRule(req.params.id,req.siteId)});});


app.get('/api/important-info',requireAdmin,async(req,res)=>{try{res.json({ok:true,items:await listImportantInfo({activeOnly:false,type:String(req.query.type||''),limit:1500,siteId:req.siteId})});}catch(e){res.status(500).json({ok:false,error:e.message});}});
app.post('/api/important-info',requireAdmin,async(req,res)=>{try{res.json({ok:true,item:await createImportantInfo({...req.body,siteId:req.siteId})});}catch(e){res.status(400).json({ok:false,error:e.message});}});
app.put('/api/important-info/:id',requireAdmin,async(req,res)=>{try{const item=await updateImportantInfo(req.params.id,req.body||{},req.siteId);if(!item)return res.status(404).json({ok:false,error:'IMPORTANT_INFO_NOT_FOUND'});res.json({ok:true,item});}catch(e){res.status(400).json({ok:false,error:e.message});}});
app.delete('/api/important-info/:id',requireAdmin,async(req,res)=>{res.json({ok:await deleteImportantInfo(req.params.id,req.siteId)});});

app.get('/api/human-requests',requireAdmin,async(req,res)=>{const status=String(req.query.status||'OPEN').toUpperCase();res.json({ok:true,items:await listHumanRequests(status,300,req.siteId)});});
...
app.get('/api/human-bridge/settings',requireAdmin,async(req,res)=>{
  ...
  const pack=x.routes?.__sites?.[req.siteId]||null;
  const routes=pack?.routes||(req.siteId==='default'?x.routes||{}:{});
  const defaultChatId=pack?.defaultChatId||(req.siteId==='default'?x.defaultChatId||'':'');
  res.json({ok:true,siteId:req.siteId,configured:Boolean(token),botTokenMasked:maskSecret(token),botUsername:x.botUsername||bridgeStatus().botUsername||null,defaultChatId,routes,enabled:Boolean(x.enabled),categories:CATEGORIES,status:bridgeStatus()});
});
app.put('/api/human-bridge/settings',requireAdmin,async(req,res)=>{
  try{
    ...
    const old=await getTelegramSettingsInternal(); const fullRoutes=(old.routes&&typeof old.routes==='object')?structuredClone(old.routes):{};
    fullRoutes.__sites=(fullRoutes.__sites&&typeof fullRoutes.__sites==='object')?fullRoutes.__sites:{};
    fullRoutes.__sites[req.siteId]={defaultChatId,routes};
    if(req.siteId==='default'){ for(const c of CATEGORIES) fullRoutes[c]=routes[c]; }
    const saved=await saveTelegramSettings({botTokenEnc:enc,botUsername:username,defaultChatId:req.siteId==='default'?defaultChatId:old.defaultChatId,routes:fullRoutes,enabled:null});
    res.json({ok:true,siteId:req.siteId,configured:Boolean(saved.botToken),botUsername:saved.botUsername,defaultChatId,routes,enabled:saved.enabled});
...
app.get('/api/human-bridge/tickets',requireAdmin,async(req,res)=>{res.json({ok:true,items:await listBridgeTickets(300,req.siteId),status:bridgeStatus()});});
...
app.get('/api/conversations',requireAdmin,async(req,res)=>{const r=await pool.query(`SELECT c.chat_id,c.customer_name,c.customer_email,c.status,c.handling_mode,c.takeover_reason,c.takeover_at,c.last_event_at,c.last_member_event_at,c.last_ai_event_at,c.workflow_type,c.workflow_state,c.member_typing,c.member_typing_updated_at,c.created_at,c.updated_at,
  ...
  WHERE c.visible_in_inbox=true AND c.site_id=$1 ORDER BY c.lc_inbox_rank ASC NULLS LAST, COALESCE(c.last_event_at,c.updated_at) DESC LIMIT 200`,[req.siteId]);res.json({ok:true,items:r.rows});});
...
app.get('/api/learning',requireAdmin,async(req,res)=>{
  ...
  res.json({ok:true,items:await listLearningExamples(status,400,req.siteId),stats:await learningStats(req.siteId)});
});
...
app.post('/api/conversations/:id/feedback',requireAdmin,async(req,res)=>{
  try{const correction=sanitizeCorrection(req.body?.correction);const eventId=String(req.body?.eventId||'').trim();if(!eventId||!correction)return res.status(400).json({ok:false,error:'EVENT_ID_AND_CORRECTION_REQUIRED'});const item=await addAiCorrection({chatId:req.params.id,aiEventId:eventId,correction});res.json({ok:true,item,note:'Teguran langsung aktif sebagai contoh koreksi untuk chat berikutnya.'});}catch(e){res.status(400).json({ok:false,error:e.message});}
});

app.get('/api/rules',requireAdmin,async(req,res)=>{const r=await pool.query(`SELECT * FROM ai_rules WHERE site_id=$1 ORDER BY id DESC`,[req.siteId]);res.json({ok:true,items:r.rows});});
app.post('/api/rules',requireAdmin,async(req,res)=>{const {category='GLOBAL',ruleType='FORBID',content}=req.body||{};if(!content)return res.status(400).json({ok:false,error:'CONTENT_REQUIRED'});const r=await pool.query(`INSERT INTO ai_rules(category,rule_type,content,site_id) VALUES($1,$2,$3,$4) RETURNING *`,[category,ruleType,content,req.siteId]);res.json({ok:true,item:r.rows[0]});});
app.delete('/api/rules/:id',requireAdmin,async(req,res)=>{await pool.query('DELETE FROM ai_rules WHERE id=$1 AND site_id=$2',[req.params.id,req.siteId]);res.json({ok:true});});
app.get('/api/knowledge',requireAdmin,async(req,res)=>{const r=await pool.query(`SELECT * FROM knowledge_base WHERE site_id=$1 ORDER BY id DESC`,[req.siteId]);res.json({ok:true,items:r.rows});});
app.post('/api/knowledge',requireAdmin,async(req,res)=>{const {category='GENERAL',title,content}=req.body||{};if(!title||!content)return res.status(400).json({ok:false,error:'TITLE_CONTENT_REQUIRED'});const r=await pool.query(`INSERT INTO knowledge_base(category,title,content,site_id) VALUES($1,$2,$3,$4) RETURNING *`,[category,title,content,req.siteId]);res.json({ok:true,item:r.rows[0]});});
app.delete('/api/knowledge/:id',requireAdmin,async(req,res)=>{await pool.query('DELETE FROM knowledge_base WHERE id=$1 AND site_id=$2',[req.params.id,req.siteId]);res.json({ok:true});});
...
app.post('/webhooks/livechat',async(req,res)=>{
  try{
    const p=req.body||{};
    const requestedSite=String(req.headers['x-livechat-site']||req.query?.site||p.site_id||'default').toLowerCase().replace(/[^a-z0-9_-]+/g,'-')||'default';
    const webhookSecret=lc.getWebhookSecret(requestedSite);
    if(webhookSecret){
    ...
    const nativeChatId=String(p.chat_id||p.chat?.id||''); const ev=p.event||p;
    if(!nativeChatId || !ev?.text) return res.status(202).json({ok:true,ignored:true});
    const chatId=lc.scopedId(requestedSite,nativeChatId); const eventId=String(ev.id||crypto.createHash('sha1').update(`${nativeChatId}|${ev.created_at||''}|${ev.text}`).digest('hex'));
    const {recordEventInbox}=await import('./db.js'); const fresh=await recordEventInbox({source:'webhook',siteId:requestedSite,chatId,eventId,payload:p});
    if(!fresh) return res.json({ok:true,duplicate:true});
    await pool.query(`INSERT INTO conversations(chat_id,site_id,native_chat_id,updated_at) VALUES($1,$2,$3,now()) ON CONFLICT(chat_id) DO UPDATE SET site_id=EXCLUDED.site_id,native_chat_id=EXCLUDED.native_chat_id,updated_at=now()`,[chatId,requestedSite,nativeChatId]);
    const type=String(ev.author_type||'').toLowerCase().includes('customer')?'customer':'agent';
    if(type==='customer') await processCustomerMessage({chatId,eventId,text:String(ev.text),createdAt:ev.created_at||new Date().toISOString(),livechat:lc});
    res.json({ok:true,siteId:requestedSite});
...
async function boot(){
  try{
    ...
    { const {getSiteProfileById,saveSiteProfile}=await import('./db.js'); for(const site of lc.siteList()){ const old=await getSiteProfileById(site.id); await saveSiteProfile(site.id,{name:old.name||site.name,loginUrl:old.loginUrl||site.loginUrl||'',registerUrl:old.registerUrl||site.registerUrl||'',notes:old.notes||''}); } }
