...
import { cannedStats, listCannedResponses } from './db.js';
...
app.get('/api/status',requireAdmin,async(req,res)=>{
  ...
  res.json({ok:true,db:dbOk,livechatConfigured:lc.ready(),openaiConfigured:ai.ready(),autoReply:Boolean(await getSetting('auto_reply',false)),poller:pollerStatus(),cannedSync:cannedSyncStatus(),canned:await cannedStats(),model:config.openaiModel,warnings:validateConfig()});
});
...
app.get('/api/canned',requireAdmin,async(req,res)=>{res.json({ok:true,stats:await cannedStats(),items:await listCannedResponses(500)});});
