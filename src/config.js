...
export const config = {
  ...
  lcPollMs: Math.max(1000, int(process.env.LIVECHAT_POLL_MS, 5000)),
  lcListLimit: Math.min(100, Math.max(1, int(process.env.LIVECHAT_LIST_LIMIT, 100))),
  // Match the LiveChat application's "My chats" by default. Use all only when explicitly requested.
  lcInboxMode: process.env.LIVECHAT_INBOX_MODE || 'my_active',
  // Even when list_chats summary looks unchanged, periodically re-read chat detail.
  // This prevents a stale provider summary from freezing new messages in the dashboard.
  lcDetailRefreshMs: Math.max(1500, int(process.env.LIVECHAT_DETAIL_REFRESH_MS, 3500)),
...
export function validateConfig() {
  ...
  if (!config.lcAccountId || !config.lcPat) warnings.push('LiveChat belum dikonfigurasi: isi LIVECHAT_ACCOUNT_ID dan LIVECHAT_PAT');
