...
const lastDetailFetchAt = new Map();
export function pollerStatus(){ return {running,paused,lastTick,lastError,lastResult,mode:config.lcSyncMode,pollMs:config.lcPollMs,detailRefreshMs:config.lcDetailRefreshMs,trackedChats:summaryFingerprints.size}; }
...
export async function syncOnce(livechat,{manual=false}={}){
  ...
  try {
    ...
    for (const [rank, summary] of chats.entries()) {
      ...
      // Never trust an unchanged list_chats summary forever. LiveChat can return a cached/stale
      // summary while the actual chat already has a newer event. Force a detail reconciliation on
      // a small TTL so the dashboard cannot freeze on an old message.
      const nowMs=Date.now();
      const lastDetail=Number(lastDetailFetchAt.get(chatId)||0);
      const detailDue=(nowMs-lastDetail)>=config.lcDetailRefreshMs;
      if (dbState?.bootstrapped_at && Number(dbState?.message_count||0) > 0 && oldFp === fp && !detailDue) { unchanged++; continue; }

      let chat=summary;
      const mustFetchDetail=detailDue || !Array.isArray(chat?.threads) || chat.threads.length===0;
      if (mustFetchDetail) {
        try { chat=await livechat.getChat(chatId, summary); fetched++; lastDetailFetchAt.set(chatId,Date.now()); }
      ...
      for (const ev of unseen) {
        const siteId=db.siteFromChatKey(chatId);
        const accepted=await db.recordEventInbox({source:'poller',siteId,chatId,eventId:ev.eventId,payload:{text:ev.text,createdAt:ev.createdAt,authorId:ev.authorId,authorType:ev.authorType,threadId:ev.threadId}}).catch(()=>true);
        // event_inbox is an audit/dedupe ledger. Message idempotency remains the final guard, so polling can recover a webhook that died mid-processing.
        const type=senderType(ev,chat);
    ...
    if(Array.isArray(data?.errors)&&data.errors.length){
      // A partial multi-site outage must never make another website's inbox disappear.
      await db.logError('poller','PARTIAL_SITE_SYNC','One or more LiveChat sites failed; inbox reconciliation skipped for this cycle',{errors:data.errors});
    }else{
      ...
      for(const id of lastDetailFetchAt.keys()) if(!seenSet.has(id)) lastDetailFetchAt.delete(id);
    }
...
export function startPoller(livechat){
  ...
  const run=async()=>{ try{await syncOnce(livechat);}catch(e){ console.warn('LiveChat poll sync failed:',e?.message||e); } finally{timer=setTimeout(run,config.lcPollMs);} };
