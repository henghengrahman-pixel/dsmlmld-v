...
async function ingestAgentEvent(chatId, ev, chat, {allowTakeover=true}={}) {
  ...
  const inserted=await db.insertMessage({chatId,eventId:ev.eventId,senderType:ours?'ai':'agent',authorId:ev.authorId,text:ev.text,normalizedText:normalizeText(ev.text),intent:detectIntent(ev.text),createdAt:ev.createdAt,attachments:ev.attachments||[]});
