...
function ticketKeyboard(ticket){
  ...
  if(ticket.category==='DEPOSIT_PROBLEM') return {inline_keyboard:[[ {text:'DP MASUK',callback_data:d('DP_PROCESSED')},{text:'DP BELUM MASUK',callback_data:d('DP_NOT_FOUND')} ]]};
...
async function handleUpdate(u,livechat){
  ...
  try{
    const simple=answer.toLowerCase().trim();
    if(ticket.category==='DEPOSIT_PROBLEM' && actionHandler && /^(?:dp\s*)?(?:masuk|sudah masuk|sudah diproses|proses)$/i.test(simple)){
      await actionHandler({request,action:'DP_PROCESSED',livechat});
    } else if(ticket.category==='DEPOSIT_PROBLEM' && actionHandler && /^(?:dp\s*)?(?:belum masuk|tidak masuk|ga masuk|gak masuk|belum)$/i.test(simple)){
      await actionHandler({request,action:'DP_NOT_FOUND',livechat});
    } else {
