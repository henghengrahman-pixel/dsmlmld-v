...
function ticketKeyboard(ticket,request=null){
  ...
  if(ticket.category==='RESET_PASSWORD'){
    ...
    if(q.includes('member sudah deposit')) return {inline_keyboard:[[ {text:'DANA belum masuk',callback_data:d('RESET_DEPOSIT_NOT_IN')} ]]};
    return {inline_keyboard:[
      [{text:'Deposit dahulu',callback_data:d('RESET_DEPOSIT_FIRST')}],
      [{text:'Rekening tidak terdaftar',callback_data:d('RESET_NOT_REGISTERED')}]
    ]};
  }
  // DP MASUK alias: tombol operasional saat ini ditampilkan sebagai DP SUDAH.
  if(ticket.category==='DEPOSIT_PROBLEM') return {inline_keyboard:[[
    {text:'DP SUDAH',callback_data:d('DP_PROCESSED')},{text:'DP BELUM MASUK',callback_data:d('DP_NOT_FOUND')}
  ]]};
