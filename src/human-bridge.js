...
async function handleCallback(u,livechat){
  ...
  if(!authorizedTelegramUser(q.from)) return true;
  ...
  if(!before || (before.telegram_message_id && String(q.message?.message_id||'')!==String(before.telegram_message_id))){ await tg.answerCallbackQuery(q.id,'Tombol ticket sudah tidak valid').catch(()=>{}); return true; }
