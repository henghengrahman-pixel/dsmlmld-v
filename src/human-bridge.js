...
function routeFor(settings,category,siteId='default'){
  const root=settings.routes&&typeof settings.routes==='object'?settings.routes:{};
  const sitePack=root.__sites&&typeof root.__sites==='object'?root.__sites[String(siteId||'default')]:null;
  const routes=sitePack?.routes&&typeof sitePack.routes==='object'?sitePack.routes:root;
  const defaultChatId=String(sitePack?.defaultChatId||settings.defaultChatId||'').trim();
  const aliases={
  ...
  let r={};
  for(const key of aliases){ if(routes[key]&&typeof routes[key]==='object'){ r=routes[key]; break; } }
  return {chatId:String(r.chatId||defaultChatId||'').trim(),topicId:String(r.topicId||'').trim()||null};
}
...
export async function dispatchHumanRequest(request){
  try{
    ...
    const category=bridgeCategory(request.intent); const siteId=String(request.site_id||db.siteFromChatKey(request.chat_id)||'default'); const route=routeFor(settings,category,siteId);
