...
export async function getCurrentSessionContext(chatId, limit=10000){
  const max=Math.max(1,Math.min(Number(limit)||10000,20000));
  const r=await pool.query(`
    WITH boundary AS (
      ...
      WHERE chat_id=$1
        ...
        AND (intent='GREETING_TRIGGER' OR lower(COALESCE(text,'')) LIKE '%lebih mudah menghubungi kami via telegram%')
    )
    SELECT m.id,m.event_id,m.sender_type,m.author_id,m.text,m.normalized_text,m.intent,m.attachments,m.created_at
    FROM messages m,boundary b
    WHERE m.chat_id=$1 AND m.id>b.id
...
export async function closeOpenHumanRequests(chatId,reason='MEMBER_RESOLVED'){
  const r=await pool.query(`UPDATE human_requests SET status='CANCELLED',closed_reason=$2,updated_at=now() WHERE chat_id=$1 AND status='OPEN' RETURNING id`,[chatId,String(reason||'MEMBER_RESOLVED').slice(0,120)]);
  if(r.rowCount){
    const ids=r.rows.map(x=>x.id);
...
export async function beginNewConversationSession(chatId,{triggerEventId=null,threadKey=null}={}){
  // New LiveChat welcome/promo banner = new service session. Keep message history for
  // audit/learning, but clear transient case state so old DP/WD/bonus decisions cannot leak.
  ...
  try{
    ...
    await pool.query(`UPDATE conversations SET
      ...
      workflow_state=NULL,
      workflow_data='{}'::jsonb,
      ai_waiting_human=false,
      handling_mode='AI',
      takeover_reason=NULL,
      takeover_at=NULL,
      human_takeover_until=NULL,
      case_brain='{}'::jsonb,
      brain_message_count=0,
      brain_updated_at=NULL,
      conversation_digest='',
      digest_message_count=0,
      digest_updated_at=NULL,
      session_key=$2,
      case_version=case_version+1,
      last_topic=NULL,
      last_reply_hash=NULL,
      last_reply_at=NULL,
      updated_at=now()
      WHERE chat_id=$1`,[chatId,sessionKey]);
    await pool.query('COMMIT');
  }catch(e){
    await pool.query('ROLLBACK').catch(()=>{});
    throw e;
  }
  await logError('engine','NEW_SESSION_BOUNDARY','LiveChat welcome/promo started a new conversation session',{chatId,triggerEventId,threadKey});
  return true;
}
...
export async function listSlaBreachedBridgeTickets(limit=50){
  // Escalation stages: 5m first reminder, 10m warning, 20m escalation.
  const r=await pool.query(`
    SELECT *,
      CASE
        WHEN created_at<=now()-interval '20 minutes' THEN 3
        WHEN created_at<=now()-interval '10 minutes' THEN 2
        WHEN created_at<=now()-interval '5 minutes' THEN 1
        ELSE 0
      END AS due_stage
    FROM human_bridge_tickets
    WHERE status='OPEN'
      AND (
        (created_at<=now()-interval '5 minutes' AND sla_stage<1)
        OR (created_at<=now()-interval '10 minutes' AND sla_stage<2)
        OR (created_at<=now()-interval '20 minutes' AND sla_stage<3)
      )
