...
export async function claimGreeting(chatId, cooldownHours=8){
  const r=await pool.query(`UPDATE conversations SET greeting_sent_at=now(),updated_at=now()
    WHERE chat_id=$1 AND (greeting_sent_at IS NULL OR greeting_sent_at < now()-($2||' hours')::interval)
    RETURNING greeting_sent_at`,[chatId,String(cooldownHours)]);
  return r.rowCount>0;
}
