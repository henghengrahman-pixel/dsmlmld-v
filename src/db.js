...
export async function getConversationState(chatId){ const r=await pool.query(`SELECT c.chat_id,c.bootstrapped_at,c.human_takeover_until,c.handling_mode,c.takeover_reason,c.takeover_at,c.last_event_at,c.greeting_sent_at,c.workflow_type,c.workflow_state,c.workflow_data,c.member_typing,c.member_typing_updated_at,(SELECT count(*)::int FROM messages m WHERE m.chat_id=c.chat_id) AS message_count FROM conversations c WHERE c.chat_id=$1`,[chatId]); return r.rows[0]||null; }
...
export async function captureHumanReplyLearning({chatId,eventId,responseText}){
  const prev=await pool.query(`SELECT text,intent,event_id FROM messages WHERE chat_id=$1 AND sender_type='customer' AND created_at <= COALESCE((SELECT created_at FROM messages WHERE chat_id=$1 AND event_id=$2),now()) ORDER BY created_at DESC LIMIT 1`,[chatId,String(eventId||'')]);
  ...
  if(existing.rowCount){
    const r=await pool.query(`UPDATE learning_examples SET occurrences=occurrences+1,updated_at=now() WHERE id=$1 RETURNING *`,[existing.rows[0].id]);
    return r.rows[0];
  }
  const r=await pool.query(`INSERT INTO learning_examples(source_type,intent,member_text,response_text,status,chat_id,source_event_id) VALUES('HUMAN_CHAT',$1,$2,$3,'PENDING',$4,$5) ON CONFLICT(chat_id,source_event_id) DO NOTHING RETURNING *`,[intent,String(member.text).trim(),String(responseText).trim(),chatId,String(eventId||'')]);
  return r.rows[0]||null;
}
