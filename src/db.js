...
export async function upsertConversation(chat,{visible=true,state={}}={}){
  ...
  await pool.query(`INSERT INTO conversations(chat_id,customer_name,customer_email,last_event_at,updated_at,visible_in_inbox,lc_is_followed,lc_active,lc_routing_status,lc_summary,lc_inbox_rank,inbox_last_seen_at,lc_thread_id,lc_session_started_at)
    VALUES($1,$2,$3,now(),now(),$4,$5,$6,$7,$8::jsonb,$9,CASE WHEN $4::boolean THEN now() ELSE NULL END,$10::text,CASE WHEN $10::text IS NOT NULL THEN now() ELSE NULL END)
    ...
    [String(chat.id), customer.name||customer.email||null, customer.email||null, Boolean(visible), state.followed ?? null, state.active ?? null, state.routingStatus||null, JSON.stringify(chat||{}), rank, threadId]);
}
...
export async function getAutoHistoryLearning(query,intent,limit=8){
  ...
  const r=await pool.query(`SELECT intent,sample_member,sample_response,occurrences,safe_for_reply,last_seen_at FROM cs_learning_patterns WHERE (intent=$1 OR intent='GENERAL') AND safe_for_reply=true ORDER BY occurrences DESC,last_seen_at DESC LIMIT 500`,[it]);
