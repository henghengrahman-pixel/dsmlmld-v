...
export async function upsertConversation(chat,{visible=true,state={}}={}){
  ...
  await pool.query(`INSERT INTO conversations(chat_id,customer_name,customer_email,last_event_at,updated_at,visible_in_inbox,lc_is_followed,lc_active,lc_routing_status,lc_summary,lc_inbox_rank)
    VALUES($1,$2,$3,now(),now(),$4,$5,$6,$7,$8::jsonb,$9)
    ON CONFLICT(chat_id) DO UPDATE SET customer_name=COALESCE(EXCLUDED.customer_name,conversations.customer_name),customer_email=COALESCE(EXCLUDED.customer_email,conversations.customer_email),updated_at=now(),visible_in_inbox=EXCLUDED.visible_in_inbox,lc_is_followed=EXCLUDED.lc_is_followed,lc_active=EXCLUDED.lc_active,lc_routing_status=EXCLUDED.lc_routing_status,lc_summary=EXCLUDED.lc_summary,lc_inbox_rank=COALESCE(EXCLUDED.lc_inbox_rank,conversations.lc_inbox_rank)`,
...
export async function findDepositResponseForBank(bank=''){
  ...
  // Operational fallback requested: when SEABANK (or an unavailable bank) has no matching response, prefer BCA.
  const bca=rows.find(x=>/bca/i.test(`${x.shortcut||''} ${x.title||''} ${x.category||''} ${(x.tags||[]).join(' ')} ${x.content||''}`));
  return bca||null;
}
...
export async function captureHumanReplyLearning({chatId,eventId,responseText}){
  ...
  if(sig){
    await pool.query(`INSERT INTO cs_learning_patterns(intent,response_signature,sample_member,sample_response,occurrences,safe_for_reply) VALUES($1,$2,$3,$4,1,$5)
      ON CONFLICT(intent,response_signature) DO UPDATE SET occurrences=cs_learning_patterns.occurrences+1,sample_member=EXCLUDED.sample_member,sample_response=EXCLUDED.sample_response,safe_for_reply=(cs_learning_patterns.safe_for_reply OR EXCLUDED.safe_for_reply),last_seen_at=now()`,
...
export async function markConversationEnded(chatId){
  const r=await pool.query(`UPDATE conversations SET status='closed',visible_in_inbox=false,lc_active=false,lc_routing_status='closed',workflow_type=NULL,workflow_state=NULL,workflow_data='{}'::jsonb,ai_waiting_human=false,updated_at=now() WHERE chat_id=$1 RETURNING chat_id,status,visible_in_inbox`,[chatId]);
