...
export async function getConversationState(chatId){ const r=await pool.query(`SELECT c.chat_id,c.bootstrapped_at,c.human_takeover_until,c.handling_mode,c.takeover_reason,c.takeover_at,c.last_event_at,c.greeting_sent_at,(SELECT count(*)::int FROM messages m WHERE m.chat_id=c.chat_id) AS message_count FROM conversations c WHERE c.chat_id=$1`,[chatId]); return r.rows[0]||null; }
