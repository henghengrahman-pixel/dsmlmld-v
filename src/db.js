...
export async function upsertConversation(chat){
  const users=chat?.users||[]; const customer=users.find(u=>u.type==='customer')||users[0]||{};
  await pool.query(`INSERT INTO conversations(chat_id,customer_name,customer_email,last_event_at,updated_at) VALUES($1,$2,$3,now(),now())
    ...
    [String(chat.id), customer.name||customer.email||null, customer.email||null]);
}
export async function insertMessage({chatId,eventId,senderType,authorId,text,normalizedText,intent,createdAt}){
  ...
  if (r.rowCount) {
    await pool.query(`UPDATE conversations SET last_event_at=$2, last_member_event_at=CASE WHEN $3='customer' THEN $2 ELSE last_member_event_at END, updated_at=now() WHERE chat_id=$1`,[chatId,createdAt,senderType]);
  }
  return r.rowCount>0;
}
export async function getContext(chatId, limit=24){
  const r=await pool.query(`SELECT sender_type,text,created_at FROM messages WHERE chat_id=$1 ORDER BY created_at DESC LIMIT $2`,[chatId,limit]);
...
export async function isHumanTakeover(chatId){ const r=await pool.query(`SELECT human_takeover_until FROM conversations WHERE chat_id=$1`,[chatId]); const d=r.rows[0]?.human_takeover_until; return d && new Date(d)>new Date(); }
