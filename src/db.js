...
export async function syncCannedResponses(items){
  ...
  try{
    ...
    for(const item of items){
      ...
      seen.push(id);
      await client.query(`INSERT INTO livechat_canned_responses(source_id,shortcut,content,tags,scope,source_updated_at,active,raw,synced_at,updated_at)
        VALUES($1,$2,$3,$4::jsonb,$5,$6,true,$7::jsonb,now(),now())
    ...
    if(seen.length) await client.query(`UPDATE livechat_canned_responses SET active=false,updated_at=now() WHERE NOT (source_id = ANY($1::text[]))`,[seen]);
...
export async function listCannedResponses(limit=500){ const r=await pool.query(`SELECT source_id,shortcut,content,tags,scope,active,source_updated_at,synced_at,updated_at FROM livechat_canned_responses ORDER BY active DESC,updated_at DESC LIMIT $1`,[limit]); return r.rows; }
...
export async function getRelevantCanned(query, limit=8){
  const r=await pool.query(`SELECT source_id,shortcut,content,tags,scope,updated_at FROM livechat_canned_responses WHERE active=true ORDER BY updated_at DESC LIMIT 1000`);
  const qTokens=new Set(cannedTokens(query));
  const scored=r.rows.map(row=>{
    const shortcut=String(row.shortcut||'').toLowerCase();
    const hayTokens=cannedTokens(`${shortcut} ${row.content} ${(row.tags||[]).join(' ')}`);
    let score=0;
