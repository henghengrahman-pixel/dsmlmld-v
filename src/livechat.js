...
export function extractChatEvents(chat) {
  ...
  // Collect any object that owns an `events` array. LiveChat has used threads,
  // last_thread and nested chat wrappers across API response variants.
  function walk(node, owner=null, depth=0) {
    ...
    if (Array.isArray(node)) {
      for (const x of node) walk(x, owner, depth+1);
      return;
    }
    for (const [k,v] of Object.entries(node)) {
      if (k === 'events') continue;
      if (v && typeof v === 'object') walk(v, node, depth+1);
    }
  }
  walk(chat);
  ...
  for (const {owner,events} of eventGroups) {
    for (const ev of events) {
      ...
      if (!text && Array.isArray(ev?.elements)) {
        text = ev.elements.map(x=>x?.title||x?.text||x?.subtitle||'').filter(Boolean).join(' ');
      }
      text = String(text || '').trim();
      if (!text) continue;
      const createdAt = ev?.created_at || owner?.created_at || new Date().toISOString();
      const ownerId = owner?.id ?? owner?.thread_id ?? '';
      const eventId = String(ev?.id ?? `${ownerId}:${createdAt}:${text}`);
      if (seen.has(eventId)) continue;
      seen.add(eventId);
      out.push({
        eventId,
        threadId: String(ownerId),
        createdAt,
        text,
