...
export class LiveChatClient {
  ...
  async test() {
    ...
    const data = await this.call('list_chats', { filters:{ include_active:true }, sort_order:'desc', limit:1 });
    return { ok:true, latencyMs:Date.now()-started, count:Array.isArray(data?.chats)?data.chats.length:0 };
  }
}
