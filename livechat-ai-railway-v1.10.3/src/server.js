...
import { migrate, healthDb, pool, getSetting, setSetting, getContext, setHumanTakeover, clearHumanTakeover, withChatLock, isHumanTakeover, logError, insertMessage, addAiCorrection, backfillHumanLearning, listLearningExamples, setLearningStatus, promoteLearningToKnowledge, promoteLearningToResponse, captureHumanReplyLearning } from './db.js';
...
app.post('/api/learning/backfill',requireAdmin,async(req,res)=>{try{res.json({ok:true,...await backfillHumanLearning(Number(req.body?.limit||5000))});}catch(e){res.status(500).json({ok:false,error:e.message});}});
...
async function boot(){
  try{
    ...
    backfillHumanLearning(5000).then(r=>console.log('CS learning backfill',r)).catch(e=>console.warn('CS learning backfill failed',e.message));
