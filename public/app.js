...
async function openChat(id,title,auto=false){
  ...
  const html=started+items.map(m=>{
    ...
    const type=originalType==='customer'?'customer':'ai';
    const label=type==='customer'?(currentTitle||'Member'):'CS';
...
function humanMessageHtml(m,name){
  ...
  const originalType=String(m.sender_type||'unknown');
  const type=originalType==='customer'?'customer':'ai';
  const label=type==='customer'?(name||'Member'):'CS';
...
async function loadHuman(){
  ...
  $$('#humanList textarea.humanDirectText').forEach(t=>t.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();const id=t.id.slice(3),btn=document.querySelector(`[data-hsend="${CSS.escape(id)}"]`);btn?.click()}}));
  $$('[data-answer]').forEach(b=>b.onclick=async()=>{const id=b.dataset.answer,answer=$('#ha-'+id).value.trim();if(!answer)return alert('Isi jawaban staff dulu.');b.disabled=true;try{await api('/api/human-requests/'+id+'/answer',{method:'POST',body:JSON.stringify({answer,saveAsKnowledge:$('#learn-'+id).checked})});humanDrafts.delete(String(id));humanLearnDrafts.delete(String(id));humanReplyDrafts.delete(String(id));humanExpanded.delete(String(id));await Promise.all([loadHuman(),loadChats(),loadLogs()])}catch(e){alert(e.message)}finally{b.disabled=false}});
