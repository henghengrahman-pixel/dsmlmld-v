...
async function expandResponseShortcut(textarea){
  ...
  try{
    const d=await api('/api/canned/shortcut?shortcut='+encodeURIComponent(raw));
...
function bindShortcutTextarea(textarea,sendButton=null){
  ...
  textarea.addEventListener('keydown',async e=>{
    if((e.key==='Tab'||e.key==='Enter')&&!e.shiftKey&&String(textarea.value||'').trim().startsWith('#')){
      e.preventDefault();
      const expanded=await expandResponseShortcut(textarea).catch(err=>{alert(err.message);return false});
      if(expanded)return;
    }
    if(e.key==='Enter'&&!e.shiftKey&&sendButton&&!String(textarea.value||'').trim().startsWith('#')){
